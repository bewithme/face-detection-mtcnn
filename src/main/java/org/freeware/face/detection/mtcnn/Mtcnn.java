...
public class Mtcnn {
	...
	public ArrayList<FaceInfo> findFace(Mat image) {
		...
		for (int i = 0; i < scales_.size(); i++) {
			...
			NetWork.nms(simpleFace_.get(i).boundingBox_, simpleFace_.get(i).bboxScore_,simpleFace_.get(i).nms_threshold, "Union");
		...
		for (int i = 0; i < firstBbox_.size(); i++) {
			if (firstBbox_.get(i).exist) {
				...
				opencv_imgproc.resize(rectImage, secImage, new Size(24, 24));
		...
		for (int i = 0; i < secondBbox_.size(); i++) {
			if (secondBbox_.get(i).exist) {
				...
				Mat rectImage = new Mat(image, temp);
				opencv_imgproc.resize(rectImage, thirdImage, new Size(48, 48));
		...
		for (int i = 0; i < thirdBbox_.size(); i++) {
			if (thirdBbox_.get(i).exist) {
				...
				// face rect
				faceInfo.setFaceRect(new Rect(thirdBbox_.get(i).y1, thirdBbox_.get(i).x1,
				...
				for (int num = 0; num < 10; num++) {
					...
					faceInfo.getKeyPoints().add(thirdBbox_.get(i).ppoint[num]);
				}
